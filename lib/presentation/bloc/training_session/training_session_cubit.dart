...
class TrainingSessionCubit extends Cubit<TrainingSessionState> {
  ...
  List<TrainingSession> _currentTrainingSessions = [];
  ...
  Future<void> loadInitialStatuses() async {
    ...
    try {
      ...
      if (_currentTrainingSessions.isNotEmpty) {
        emit(TrainingSessionLoaded(
          training_sessions: _currentTrainingSessions,
  ...
  Future<void> fetchTrainingSessions({bool forceRefresh = false}) async {
    ...
    emit(TrainingSessionLoading(training_sessions: _currentTrainingSessions, downloadStatus: _currentDownloadStatus));
    try {
      _currentTrainingSessions = await _training_sessionRepository.getTrainingSessions();
      ...
      emit(TrainingSessionError(
        ...
        training_sessions: _currentTrainingSessions, // Show old training_sessions if fetch fails
        downloadStatus: _currentDownloadStatus,
      ));
    }
  }
  ...
  Future<void> downloadTrainingSession(int training_sessionId) async {
    ...
    try {
      training_session = _currentTrainingSessions.firstWhere((p) => p.id == training_sessionId);
    } catch (e) {
    ...
    if (training_session == null) {
      emit(TrainingSessionError(
          message: "TrainingSession with ID $training_sessionId not found.", training_sessions: _currentTrainingSessions, downloadStatus: _currentDownloadStatus));
      return;
    }
    ...
    if (updatedTrainingSession.isUserCreated) {
      ...
      if (index != -1) {
        ...
        _training_sessionRepository.updateTrainingSession(updatedTrainingSession, repetitionsMap: repetitionsMap).then((_) {
          ...
          _currentTrainingSessions = updatedTrainingSessions;
