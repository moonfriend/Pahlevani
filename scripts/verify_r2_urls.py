...
verify_r2_urls.py
═════════════════
One-off check: confirm every movement.media_src value in the DB now points at
Cloudflare R2 (not Supabase Storage) and actually downloads (HTTP 200,
non-empty body) — the final verification step of the R2 media cutover (see
supabase/migrations/0006_r2_media_urls.sql).

exercise.audio_url is no longer checked here — that column was dropped by
migration 0034_drop_legacy_exercise_audio_columns.sql; audio URLs now live on
movement_audio_track instead.
...
def main() -> None:
    ...
    total_bad = i_bad_host + i_fail
    print(f"\n{'═' * 60}")
    if total_bad == 0:
        print(f"  ✓  All {i_ok} media URLs point to Cloudflare and download successfully.")
    else:
